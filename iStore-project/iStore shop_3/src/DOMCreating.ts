...
function CreatePhoneCard<T extends string>(phone_name: T, numberOfStorage: T, phoneColor: T, iphone_price:T, phone_picture:T):void{
    // создание карточки продукта :)

    let catalog_list_wrapper = document.querySelector('.test_block') as HTMLElement;
...
function GenSpans(choose_color_block:HTMLElement, phone_name: string):void{
    // параметры:
    ...
    let colors_mass = colors_obj[phone_name as keyof typeof colors_obj];
...
function GenFilterColourSettings(main_filter_block:HTMLElement, phone_characts: string, filter_checker: boolean, phone_colors_mass?: string):void{
...
// busket creating
function CreateBusketChoosenPhoneInfCatalog<T extends string>(phone_picture:T, phone_memory:T, phone_color:T, phone_price:T, phone_model:T, BasketOrComparison:BasketOrComparison, IfMobile?:T){
    let main_parent_to_push = Array.from(document.getElementsByClassName('compation_text')as HTMLCollectionOf<HTMLElement>);
    ...
    if(IfMobile){
        ...
        let mobile_basket_block = mobile_compation_text_wrapper.querySelector('.compation_text') as HTMLElement;
