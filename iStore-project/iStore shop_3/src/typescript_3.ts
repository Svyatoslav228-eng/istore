...
type FilterSetOrJustGen = 'colorFilter' | 'memoryFilter' | 'justGen';
type BasketOrComparison = 'basket' | 'comparison';
type PcVsMobile = 'PC' | 'Mobile';
...
let global_memory_mass:string[] = [];
let obj_mass_of_current_phone:object[] = [];
...
let filter_color_block = Array.from(document.getElementsByClassName('colors_to_filter_ul') as HTMLCollectionOf<HTMLUListElement>)

// обращаемся к блоку памяти
// let filter_memory_block = document.querySelector('.memory_to_filter_ul') as HTMLUListElement;

let filter_memory_block = Array.from(document.getElementsByClassName('memory_to_filter_ul') as HTMLCollectionOf<HTMLUListElement>)

// тут берем определенный массив по ключу объекта colors_obj
let color_mass = colors_obj[current_phone_name as keyof typeof colors_obj];
...
function RequestGenFuncToGenIn(filter_color_block:HTMLUListElement){
    for (let i = 0; i < color_mass.length; i++){
        for (let named_key in decoded_colors_obj) {
            ...
            if(decoded_colors_obj[named_key as keyof typeof decoded_colors_obj] === color_mass[i])
                // DOMCreating.ts   
...
function RequestToGenMemoryBlock(filter_memory_block:HTMLUListElement){
...
let color_filter_parent_block = Array.from(document.getElementsByClassName('colour_filter') as HTMLCollectionOf<HTMLElement>);


if(window.innerWidth > 1426){
    const filterCheckBoxes = Array.from(color_filter_parent_block[1].getElementsByClassName('checkBoxFilter') as HTMLCollectionOf<HTMLInputElement>);
    const colorNameBlocks = Array.from(color_filter_parent_block[1].getElementsByClassName('color_name') as HTMLCollectionOf<HTMLElement>);

    // CreatingProductCard.ts
    FilteringByColor(filterCheckBoxes, colorNameBlocks);
}
else{
    const filterCheckBoxes = Array.from(color_filter_parent_block[0].getElementsByClassName('checkBoxFilter') as HTMLCollectionOf<HTMLInputElement>);
    const colorNameBlocks = Array.from(color_filter_parent_block[0].getElementsByClassName('color_name') as HTMLCollectionOf<HTMLElement>);
...
let memory_filter_parent_block = Array.from(document.getElementsByClassName('memory_filter') as HTMLCollectionOf<HTMLElement>);

if(window.innerWidth > 1426){
    const filterCheckBoxes = Array.from(memory_filter_parent_block[1].getElementsByClassName('checkBoxFilter') as HTMLCollectionOf<HTMLInputElement>);
    const memoryNameBlocks = Array.from(memory_filter_parent_block[1].getElementsByClassName('color_name') as HTMLCollectionOf<HTMLElement>);

    // CreatingProductCard.ts
    FilterByMemory(filterCheckBoxes, memoryNameBlocks);
}
else{
    const filterCheckBoxes = Array.from(memory_filter_parent_block[0].getElementsByClassName('checkBoxFilter') as HTMLCollectionOf<HTMLInputElement>);
    const memoryNameBlocks = Array.from(memory_filter_parent_block[0].getElementsByClassName('color_name') as HTMLCollectionOf<HTMLElement>);
...
// filter by price 
let FilterPriceBtns = Array.from(document.getElementsByClassName('btn_to_filter_by_price') as HTMLCollectionOf<HTMLElement>);

for (let i = 0; i < FilterPriceBtns.length; i++) {
    FilterPriceBtns[i].addEventListener('click', ()=>{
        let inputFilterBlock = Array.from(document.getElementsByClassName('input_filter') as HTMLCollectionOf<HTMLElement>);
        let [input_from, input_to] = [inputFilterBlock[i].querySelector('.input_from') as HTMLInputElement, inputFilterBlock[i].querySelector('.input_to') as HTMLInputElement];
        ...
        for (let i = 0; i < obj_mass_of_current_phone.length; i++) {
            ...
            let getPhonePrice:string = getObj[`${current_phone_name}_price` as keyof typeof getObj];
        ...
        if(mass.length > 0){
            let block_to_dell_inf = document.querySelector('.test_block') as HTMLElement;
            block_to_dell_inf.innerText = '';
    
            let phone_storage:any; let phone_color:any;
    
            for (let i = 0; i < mass.length; i++) {
                let objToGen = mass[i];
                for (const key in objToGen) {
    
                    let getInnerObj:object[] = objToGen[key as keyof typeof objToGen];
    
                    for (const key in getInnerObj) {
                        switch(key){
                            case 'Внутренняя память':
                                phone_storage = getInnerObj[key as keyof typeof getInnerObj];
                                break;
                            case 'Цвет':
                                phone_color = getInnerObj[key as keyof typeof getInnerObj];
                                break;
                        }    
                    }
                }
    
                CreatePhoneCard(current_phone_name!, phone_storage, phone_color, objToGen[Object.keys(objToGen)[0] as keyof typeof objToGen], objToGen[Object.keys(objToGen)[1] as keyof typeof objToGen]);
            }
        }
    })
}
...
document.querySelector('.reset_filter_by_price')?.addEventListener('click',()=>{
    let block_to_dell_inf = document.querySelector('.test_block') as HTMLElement;
    block_to_dell_inf.innerText = '';
...
let buy_btns = Array.from(document.getElementsByClassName('buy_btn') as HTMLCollectionOf<HTMLButtonElement>);
...
let visual_counter =  Array.from(document.getElementsByClassName('number_of_choosen_goods') as HTMLCollectionOf<HTMLElement>);

// очиститель информации из корзины (используется после перезагрузки страницы)
let basket_block_remover =  Array.from(document.getElementsByClassName('compation_text') as HTMLCollectionOf<HTMLElement>);
let total_phone_price_block = document.querySelector('.phone_total_price_block') as HTMLElement;                    
let block_to_push_price = document.querySelector('.total_price') as HTMLElement;
...
// mobile variables
let mobile_compation_text_wrapper = document.querySelector('.compation_block_mobile') as HTMLElement;
let mobile_compaction_text = mobile_compation_text_wrapper.querySelector('.compation_text') as HTMLElement;

// let text_empty_block_remover = mobile_compaction_text.querySelector('.empty_block') as HTMLElement;

let total_price_mobile_block = mobile_compation_text_wrapper.querySelector('.phone_total_price_block') as HTMLElement;
let mobile_block_to_push_price = mobile_compation_text_wrapper.querySelector('.total_price') as HTMLElement;
let mobile_visual_basket_counter = document.querySelector('.number_of_choosen_goods_id_mobile') as HTMLElement;
...
let block_move_to_comparison = document.querySelector('.comp_block_inf') as HTMLElement;


let comp_btns = Array.from(document.getElementsByClassName('comp_btn') as HTMLCollectionOf<HTMLButtonElement>);

for (let i = 0; i < comp_btns.length; i++) {
    comp_btns[i].addEventListener('click', ()=>{

        let text_empty_block_remover = basket_block_remover[0].querySelector('.empty_block') as HTMLElement;
        ...
        let [phone_picture, phone_price, phone_storage, phone_color] = [
            ...
            get_current_phone_inf[Object.keys(get_current_phone_inf)[1] as keyof typeof get_current_phone_inf],
            get_current_phone_inf[Object.keys(get_current_phone_inf)[0] as keyof typeof get_current_phone_inf],
            '',''
        ];

        // обход и находения нужных ключей и сбор из них информации
        for (const key in get_current_phone_inf) {
            let inner_key:object = get_current_phone_inf[key as keyof typeof get_current_phone_inf];

            for (const key in inner_key) {
                if(key === 'Внутренняя память')
                    phone_storage = inner_key[key as keyof typeof inner_key];
                else if(key === 'Цвет')
                    phone_color = inner_key[key as keyof typeof inner_key];
            }   
        }
        ...
        if('comparison_basket_phones' in localStorage){
            ...
            let comparison_basket_mass:object[] = [];
...
let comparisonBtns = Array.from(document.getElementsByClassName('comparesonBtn') as HTMLCollectionOf<HTMLElement>);
...
let likeBtns = Array.from(document.getElementsByClassName('likeBtn') as HTMLCollectionOf<HTMLElement>);
...
let btn_elem_in_storage_block = Array.from(document.getElementsByClassName('btn_elem_in_storage') as HTMLCollectionOf<HTMLDivElement>);
let text_block_show = Array.from(document.getElementsByClassName('text_block') as HTMLCollectionOf<HTMLDivElement>);
...
let showModalFilterBtn = document.querySelector('.filter_btn_for_mobile') as HTMLElement;
let modalFilterBlock = document.querySelector('.modal_filter_block') as HTMLElement;
let getBody = document.querySelector('body') as HTMLBodyElement;
let closeModalFilterWindow = document.querySelector('.close_btn') as HTMLElement;
let getDardkerBackGround = document.querySelector('.darkerBackGround') as HTMLElement;
